...
(defn parse-atom
  ...
  [element]
  (cond
    (fn? element) (let [fn-element (element)]
                    (if (fn? fn-element)
                      (fn-element)
                      fn-element))
    (float? element) (if (< 0.5 (rand))
                       element
                       (loop [index 0]
                         (let [instr (:instruction (nth @adaptive-source index))]
                           (if (and (fn? instr) (float? (subparse instr)))
                             (subparse instr)
                             (recur (inc index))))))
    (int? element) (if (< 0.5 (rand))
                     element
                     (loop [index 0]
                       (let [instr (:instruction (nth @adaptive-source index))]
                         (if (and (fn? instr) (int? (subparse instr)))
                           (subparse instr)
                           (recur (inc index))))))
    :else element))

(defn pseudo-random-atom
      ...
      (zipmap markers
              (map (fn [marker]
                     (case marker
                       ...
                       :instruction (pseudo-random-atom) ; new
